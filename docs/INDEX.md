# Documentation Index

- [`docs/ai/internal-knowledge.md`](ai/internal-knowledge.md): durable repository knowledge for AI agents.
- [`docs/ai/core-memory.md`](ai/core-memory.md): durable code-smell, dead-code, and CI maintenance memory.
- [`docs/REFACTORING.md`](REFACTORING.md): refactoring notes and planning context.
- [`docs/ai/tanstack-start-css-styling.md`](ai/tanstack-start-css-styling.md): TanStack Start CSS styling patterns, SSR discovery, Early Hints, and production CSS inlining (reference for upgrade work).
