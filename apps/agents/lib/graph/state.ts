 ...
 */
function validateToolCall(tool: ToolCall, index: number): string[] {
  const errors: string[] = [];
...
function valueChanged(oldValue: unknown, newValue: unknown): boolean {
  ...
  if (Array.isArray(oldValue) && Array.isArray(newValue)) {
    ...
    return oldValue.some((v, i) => valueChanged(v, newValue[i]));
  }
 ...
 */
function toolCallChanged(old: ToolCall, new_: ToolCall): boolean {
  return (
    old.id !== new_.id ||
    old.toolName !== new_.toolName ||
    old.status !== new_.status ||
    valueChanged(old.parameters, new_.parameters) ||
    valueChanged(old.result, new_.result) ||
    valueChanged(old.error, new_.error)
  );
}

/**
 * State manager utility functions
 ...
 */
export function validate(state: AgentState): StateValidationResult {
  const errors: string[] = [];
  ...
  if (!Array.isArray(state.toolCalls)) {
    ...
    for (let i = 0; i < state.toolCalls.length; i++) {
      ...
      const toolErrors = validateToolCall(tool, i);
 ...
 */
export function clone(state: AgentState): AgentState {
  return {
...
export function computeDiff(oldState: AgentState, newState: AgentState): StateDiff {
  ...
  for (const field of fields) {
    ...
    } else if (valueChanged(oldValue, newValue)) {
 ...
 */
export function createCheckpoint(state: AgentState): AgentState {
  return {
    conversationId: state.conversationId,
    userInput: state.userInput,
    response: state.response,
 ...
 */
export function restoreFromCheckpoint(checkpoint: AgentState): AgentState {
  const validation = validate(checkpoint);
