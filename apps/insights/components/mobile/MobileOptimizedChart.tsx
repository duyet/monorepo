...
import { CompactAreaChart, CompactBarChart, CompactLineChart, MiniSparkline } from '@/components/charts/CompactChart'
...
export function MobileOptimizedChart({
  ...
  const displayCategories = shouldPaginate ? [categories[currentCategory]] : categories
  ...
  const renderDataTable = () => (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b">
            ...
            {displayCategories.map((category) => (
              <th key={category} className="text-right p-2 font-medium text-muted-foreground">
                {category}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {data.slice(0, 10).map((row, i) => ( // Limit to 10 rows on mobile
            <tr key={i} className="border-b border-border/50">
              <td className="p-2 font-medium">
                {String(row[index])}
              </td>
              {displayCategories.map((category) => (
                <td key={category} className="text-right p-2 font-mono">
            ...
            <div className="flex items-center space-x-1 shrink-0">
              ...
              <div className="flex rounded border">
                ...
                <button
                  ...
                  className={cn(
                    'px-2 py-1 text-xs rounded-r border-l',
                    currentView === 'data'
                      ? 'bg-accent text-accent-foreground'
                      : 'hover:bg-accent/50'
                  )}
