...
import { executeQuery, executeCommand, executeStatements, getClient } from './client'
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('-- @version:')) {
        version = Number.parseInt(trimmed.replace('-- @version:', '').trim(), 10)
        continue
      }
...
export class MigrationRunner {
  ...
  private loadMigrations(): Migration[] {
    console.log('[Migration Runner] Loading migrations from:', this.migrationsDir)
  ...
  async getApplied(): Promise<AppliedMigration[]> {
    ...
    const query = 'SELECT version, name, checksum, applied_at FROM monorepo_migrations ORDER BY version'
  ...
  async up(): Promise<void> {
    ...
    for (const migration of pending) {
      console.log(`[Migration Runner] Applying migration ${migration.version}: ${migration.name}`)
      ...
      try {
        ...
        console.log(`[Migration Runner] Successfully applied migration ${migration.version}`)
      } catch (error) {
        console.error(`[Migration Runner] Failed to apply migration ${migration.version}:`, error)
        throw error
      }
    }
  ...
  async down(count = 1): Promise<void> {
    ...
    for (const appliedMigration of toRollback) {
      ...
      if (!migration) {
        console.error(`[Migration Runner] Migration file not found for version ${appliedMigration.version}`)
        continue
      }

      if (!migration.down) {
        console.error(`[Migration Runner] No DOWN section for migration ${migration.version}`)
        continue
      }

      console.log(`[Migration Runner] Rolling back migration ${migration.version}: ${migration.name}`)
      ...
      try {
        ...
        if (!deleteResult.success) {
          throw new Error(`Failed to remove migration record: ${deleteResult.error}`)
        }

        console.log(`[Migration Runner] Successfully rolled back migration ${migration.version}`)
      } catch (error) {
        console.error(`[Migration Runner] Failed to rollback migration ${migration.version}:`, error)
        throw error
      }
    }
  ...
  async verify(): Promise<boolean> {
    ...
    for (const appliedMigration of applied) {
      ...
      if (!migration) {
        console.error(`[Migration Verify] Migration file not found for version ${appliedMigration.version}`)
        valid = false
        continue
      }
