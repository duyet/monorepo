...
export function LightboxTopControls({
  ...
  return (
    <div className={cn('absolute left-4 right-4 top-4 z-10 flex items-center justify-between', className)}>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="rounded-full bg-black/50 p-2.5 text-white transition-all duration-200 hover:bg-black/70 hover:scale-105 backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-white/50"
        ...
        <Dialog.Close asChild>
          <button
            className="rounded-full bg-black/50 p-2.5 text-white transition-all duration-200 hover:bg-red-500/80 hover:scale-105 backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-white/50"
...
export function NavigationButton({ direction, onClick, isFullscreen, className }: NavigationButtonProps) {
...
export function InfoPanel({ photo, metadata, isFullscreen, className }: InfoPanelProps) {
  if (isFullscreen) {
    return (
      <div className={cn(
        ...
        <div className="space-y-4">
          {photo.description && (
            <h3 className="text-xl font-semibold leading-tight">{photo.description}</h3>
          )}
          ...
          <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-gray-300">
                ...
                {metadata.stats && (
                  <p>👁 {metadata.stats.views} views • ⬇ {metadata.stats.downloads} downloads</p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-1 text-gray-300">
                ...
                {metadata.exif && (
                  <>
                    ...
                    {metadata.exif.settings && <p>⚙️ {metadata.exif.settings}</p>}
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-4 pt-3 border-t border-gray-600">
            <a
              ...
              className="flex items-center gap-2 text-sm font-medium text-blue-300 hover:text-blue-200 transition-colors"
            >
            ...
            <a
              ...
              rel="noopener noreferrer"
              download
              className="flex items-center gap-2 text-sm font-medium text-green-300 hover:text-green-200 transition-colors"
            >
