...
import { parseCsv, detectColumns, findHeaderRowIndex, getCellValue } from '../../lib/csv'
import { normalizeDate, normalizeParams, normalizeLicense, normalizeType, normalizeText } from '../../lib/normalizers'
...
export const curated: DataSourceAdapter = {
  ...
  async fetch({ verbose = false } = {}): Promise<Model[]> {
    ...
    if (verbose) {
      console.log(`  Parsed ${dataRows.length} rows → ${models.length} valid, ${skipped} skipped`)
    }
