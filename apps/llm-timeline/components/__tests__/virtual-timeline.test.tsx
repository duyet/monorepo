...
describe('VirtualTimeline', () => {
  it('renders without crashing', () => {
    ...
    modelsByYear.set(2023, [{ name: 'GPT-4', date: '2023-03-14', org: 'OpenAI' }])
    
    const { container } = render(<VirtualTimeline modelsByYear={modelsByYear} />)
    expect(container).toBeDefined()
  })

  it('renders empty state when no models match', () => {
    const modelsByYear = new Map()
    const { getByText } = render(<VirtualTimeline modelsByYear={modelsByYear} />)
