    ...
    <div
      className={`absolute top-3 left-3 z-20 flex h-6 w-6 items-center justify-center rounded-md text-xs font-bold transition-all duration-200 ${
        isActive
          ...
          : "bg-white/90 text-neutral-900 border border-neutral-200 dark:bg-neutral-900/90 dark:text-white dark:border-white/10"
      }`}
    >
      {number}
    </div>
...
export function KeyboardHelpTooltip({
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-200">
      <div className="mx-auto max-w-md rounded-xl border border-neutral-200 bg-white p-4 dark:border-white/10 dark:bg-neutral-900">
...
export function KeyboardHelpButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="fixed bottom-6 right-6 z-40 rounded-full border border-neutral-200 bg-white p-3 transition-all hover:scale-105 dark:border-white/10 dark:bg-neutral-900"
