...
10. **ai-percentage** (`apps/ai-percentage/`) - Dashboard showing the percentage of code written by AI across repositories
    ...
    - Static SPA; use `bun run build` for local builds and `bun run cf:deploy:prod` for production publish
...
- Notable app workflows:
  ...
  - `apps/api`: `bun run dev` runs Wrangler on port 8787; `bun run deploy` builds then deploys the worker; `bun run test:watch` enables watch-mode API testing.
