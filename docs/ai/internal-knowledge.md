...
- **aidr product** (website + Chrome extension + ingest) now lives in standalone repo [`duyet/aidr`](https://github.com/duyet/aidr) at **`https://aidr.today`** (Cloudflare Workers; Account ID `7df185a18b98382c3240fa7ac4a37075`, Zone ID `e89512b5edc14f2f790aa3eb47f60944`). Legacy `https://news.duyet.net` is a custom-domain alias. Do not add new features to monorepo `apps/news` / `apps/news-tab` — edit `duyet/aidr` instead. Monorepo trees remain until cutover cleanup removes them.
- `apps/news-tab` (legacy): Chrome MV3 unpacked new-tab extension formerly for `https://news.duyet.net`. Superseded by `duyet/aidr` `apps/extension` (`aidr.zip` / Load unpacked `aidr/` folder).
