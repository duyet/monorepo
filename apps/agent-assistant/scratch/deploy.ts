...
const env = {
  ...
  ...parseEnvFile(join(appPath, ".env")),
  ...parseEnvFile(join(appPath, ".env.local")),
  ...process.env,
} as any;

// Append the self-contained ThreadStateDO Durable Object to the server bundle
...
// Inject parsed environment variables into wrangler.json vars
const prodEnv = parseEnvFile(join(rootDir, ".env.production"));
