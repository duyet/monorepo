...
description: "A short note on what belongs in agent_system.md vs tool docstrings when you speak OpenAI-shape tools to OpenRouter."
---
...
LangChain `ChatOpenAI.bind_tools` emits the OpenAI function-calling shape. OpenRouter translates that to Gemini `functionDeclarations`. Nothing registers once and then drops out — the same `tools[]` is re-sent on every hop.
...
Anthropic's API is the odd one that splices tools into a constructed system prompt. We speak OpenAI-shape to OpenRouter, so `tools[]` is a real separate field.
