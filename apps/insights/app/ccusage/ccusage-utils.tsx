 ...
 */
function anonymizeProjects(projects: Record<string, unknown>[]): CCUsageProjectData[] {
 ...
 */
async function executeClickHouseQueryLegacy(
  query: string,
...
export async function getCCUsageModels(days: DateRangeDays = 30): Promise<CCUsageModelData[]> {
  ...
  return results.map((model) => ({
    ...
    percent:
      totalTokens > 0
        ? Math.round(((Number(model.total_tokens) || 0) / totalTokens) * 100)
        : 0,
    costPercent:
      totalCost > 0
        ? Math.round(((Number(model.total_cost) || 0) / totalCost) * 100)
        : 0,
