 ...
 */
export function convertRetentionPolicy(configPolicy: ConfigRetentionPolicy): RetentionPolicy {
  return {
...
async function deleteExpiredRecords(
  ...
  if (dryRun) {
    console.log(`[Retention] DRY RUN: Would delete records from ${policy.table}`)
...
export async function applyRetentionPolicies(
  ...
  console.log(`[Retention] Applying ${policies.length} retention policies (dry run: ${dryRun})`)
  ...
  for (const policy of policies) {
    ...
    try {
      ...
      if (!dryRun) {
        ...
        console.log(`[Retention] Deleted ${recordsToDelete} records from ${policy.table}`)
      } else {
        results.push({
          table: policy.table,
          recordsToDelete,
        ...
        console.log(`[Retention] DRY RUN: Would delete ${recordsToDelete} records from ${policy.table}`)
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unknown error'

      console.error(`[Retention] Failed to apply policy for ${policy.table}:`, errorMsg)
...
export async function getTableSizes(
  ...
  for (const row of result.data) {
    ...
    const rows = typeof row.rows === 'number' ? row.rows : Number.parseInt(String(row.rows), 10)
    const bytes = typeof row.bytes === 'number' ? row.bytes : Number.parseInt(String(row.bytes), 10)
...
export async function getRetentionStatus(
  ...
  for (const policy of policies) {
    try {
      ...
      const total = typeof totalRows === 'number' ? totalRows : Number.parseInt(String(totalRows), 10)

      const compliancePercentage = total > 0 ? ((total - expiredRows) / total) * 100 : 100
      ...
      console.error(`[Retention] Failed to get status for ${policy.table}:`, error)
