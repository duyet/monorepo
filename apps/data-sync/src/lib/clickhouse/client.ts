...
export function getClickHouseConfig(): ClickHouseConfig | null {
  ...
  const explicitProtocol = process.env.CH_PROTOCOL || process.env.CLICKHOUSE_PROTOCOL
...
export async function ping(): Promise<boolean> {
  ...
  try {
    ...
    console.log('[ClickHouse Ping] Connection test:', success ? 'SUCCESS' : 'FAILED')
    return success
  } catch (error) {
...
export async function executeCommand(
  ...
  console.error('[ClickHouse Command] All retries exhausted:', lastError?.message)
  return {
    success: false,
    data: [],
...
export async function executeStatements(
  ...
  try {
    for (const statement of statements) {
      ...
      console.log('[ClickHouse Execute] Running statement:', trimmed.substring(0, 100))
    ...
    console.error('[ClickHouse Execute] Failed to execute statements:', errorMsg)
    return {
      success: false,
      data: [],
      error: errorMsg,
