...
import { m as models, g as getSlug } from "./router-Dyrqdy-I.js";
import { p as parseParamValue, f as formatDate, g as getLicenseColor, a as getLicenseBarColor } from "./utils-BYY1hTdF.js";
import "next-themes";
import "fs";
const MAX_COMPARE = 4;
const MIN_COMPARE = 2;
  ...
  const updateUrl = (next) => {
    navigate({
      search: next.length > 0 ? {
        models: next.map((m) => getSlug(m.name)).join(",")
      } : {},
      replace: true
    });
  };
