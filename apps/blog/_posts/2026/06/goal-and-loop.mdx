...
x: https://x.com/_duyet/status/2069746255333654869
...
- You give your AI a prompt, the AI spits out an output. This is **Generative AI**. `Edit this file, add this test function` is an instruction prompt.
- You give your AI a goal and a loop, and it keeps working until it meets that goal, learns from it, proactively proposes next steps. This is **Agentic AI**. `Ship the refactor in small PRs, deploy each one, don't break master` is a goal.
...
[ClickHouse Monitoring (Migrate Nextjs to Tanstack Start)](https://blog.duyet.net/2026/06/migrate-tanstack/) took ~37h of autonomous work to complete with a lot of improvements. There are ~50 PRs merged by daily on [https://anyrouter.dev](https://anyrouter.dev).

## A goal and a loop

Two parts: a goal the agent reads, and a loop that keeps going until the goal is met.

<div className="full-bleed max-w-[560px]">
<Mermaid>{`
flowchart LR
    Work["Work a pass"] --> Goal{"Goal<br/>met?"}
    Goal -->|no| Work
    Goal -->|yes| Done["Done ✓"]
`}</Mermaid>
</div>

The two words pull in different directions, and that's the point:

- A **goal** is a loop with a finish line. It has target conditions — a *done when* — so it works, checks itself, and stops the moment the conditions hold. `ship the refactor, tests green, deploy each PR` finishes.
- A **loop** doesn't have to finish. Some work is a recurrence, not a target: `every 30m, babysit and review every open PR, merge what's green`.

Most of what I run is the first kind. The second kind is for the maintenance that never really ends — watching PRs, triaging issues, keeping master green — where the schedule *is* the goal.
...
The same loop, now with a memory wrapped around it — read the kb before a pass, write what it learned after:

<div className="full-bleed max-w-[640px]">
<Mermaid>{`
flowchart LR
    KB[("kb")] -. read before .-> Work["Work a pass"]
    Work --> Goal{"Goal<br/>met?"}
    Goal -->|"no · self-fix"| Work
    Goal -->|yes| Done["Done ✓"]
    Work -. write findings .-> KB
`}</Mermaid>
</div>
