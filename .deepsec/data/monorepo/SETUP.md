# Agent setup for `monorepo`

This is a deepsec scanning workspace. Project `monorepo` was just registered
(target: `..`). Setup is incomplete — `data/monorepo/INFO.md`
...
2. **Fill in `data/monorepo/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id monorepo
pnpm deepsec process --project-id monorepo
```
