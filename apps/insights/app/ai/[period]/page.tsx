...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    description: isAllTime
      ...
      : `AI usage analytics for the last ${config.label}`,
  };
}
...
export default async function AIUsagePeriodPage({ params }: PageProps) {
  ...
  // Helper to get appropriate section title for "all" period
  const getSectionTitle = (section: (typeof SECTION_CONFIGS)[number]) => {
    if (isAllTime || days === 365) {
      if (section.id === "activity") return "Monthly Activity";
      if (section.id === "costs") return "Monthly Costs";
      if (section.id === "daily-table") return "Monthly Usage Detail";
    }
    return section.title;
  };

  // Helper to get appropriate description for "all" period
  const getSectionDescription = (title: string) => {
    return isAllTime
      ? `All-time ${title.toLowerCase()}`
      : `${title} for the last ${config.label}`;
  };
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-8">
        {SECTION_CONFIGS.map((section) => {
          ...
          const description = getSectionDescription(sectionTitle);
